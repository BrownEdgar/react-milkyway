    ...
    <div className='App'>
        {Object.keys( formik.touched).length> 0 && (
            ...
            {Object.keys(formik.touched).map(inputName =>{
                return  formik.errors[inputName] ? <p className='error'>{formik.errors[inputName]}</p> :null
            })}
            </div>
        )}
        ...
        <form  onSubmit={formik.handleSubmit}>
            ...
            <div>
                ...
                <input onBlur={formik.handleBlur} type="password" id='password' name='password' onChange={formik.handleChange}/>

            </div>
            <div>
                <input type="submit"  className='button-64 '/>
            </div>
        </form>
    </div>
  )
